...
#define ETB_EBADMESSAGE                  -219        /**< a bad message is given */
 ...
 *-----------------------------------------------------------------------------*/
	...
	#define ETB_MAGIC_INFO_DRV_7             0xC6        /* denotes the seventh drive information request (record 12) */
	...
	#define ETB_MAGIC_INFO_MASTER_1			 0x20        /* denotes a master specific information request (record 12) */
#endif /* ETB_OO_API */
...
#ifdef F_ITRON
	#define _ETB_EXPORT __cdecl
	#define ETB_CALLBACK __cdecl
#endif /*F_ITRON*/
...
/* 64 bits integer			*/
	...
	/*------------------------------*/
	/* POSIX, F_ITRON, VXWORKS_6_964 bits integer		*/
...
typedef struct ETB_DRV_INFO {
    ...
    int serial_number;             /**< The serial number of drive */
	...
	int nb_axis;					/**< The number of axis managed by the drive */
	bool metadata;					/**< TRUE if the drive is able to return metadata */	
#else /* ETB_OO_API */
    int productNumber;              /**< The drive product number */
    int bootRevision;               /**< The boot revision of drive */
    int serialNumber;              /**< The serial number of drive */
    dword softVersion;              /**< The version of drive software */
    dword productString[8];
    bool true_gantry;				/**< The gantry is a true gantry */
    int hwVersion;					/**< The hardware version */
	int nbAxis;						/**< The number of axis managed by the drive */
	bool metadata;					/**< TRUE if the drive is able to return metadata */	
	#endif /* ETB_OO_API */
} ETB_DRV_INFO;
...
typedef struct ETB_MASTER_INFO {
    int size;                    /**< The size of this structure */
	#ifndef ETB_OO_API
	int stream_max_data_size;		/**< The maximal number of long of a record used for stream on download*/
	int upl_stream_max_data_size;		/**< The maximal number of long of a record used for stream on upload*/
	int transnet_frame_cfg;			/**< The transnet configuration 0:100 us, 1:50 us */
	#else /* ETB_OO_API */
	int streamMaxDataSize;			/**< The maximal number of long of a record used for stream on download*/
	int uplStreamMaxDataSize;		/**< The maximal number of long of a record used for stream on upload*/
	int transnetFrameCfg;			/**< The transnet configuration 0:100 us, 1:50 us */
	#endif /* ETB_OO_API */
} ETB_MASTER_INFO;
...
typedef struct ETB_EXT_INFO {
    ...
    int serial_number;             /**< The serial number of extension card */
    ...
    int serialNumber;              /**< The serial number of extension card */
...
eint64  _ETB_EXPORT etb_get_timer(void);
...
int	    _ETB_EXPORT etb_set_policy(ETB *etb, int policy, int prio);
...
int		_ETB_EXPORT etb_read_slot_array(ETB *etb, int nb_slots, int slots[], dword *val[]);
int		_ETB_EXPORT etb_write_slot_array(ETB *etb, int nb_slots, int slot[], dword val[]);
...
int     _ETB_EXPORT etb_etcom_putm_unsafe(ETB_PORT *port, const void *key, eint64 mask, ETB_CONST_REC ETB_ETCOM *rec, long timeout);
/*------------------------------------------------------------------------------
...
int     _ETB_EXPORT etb_etcom_start_download_ex(ETB *etb, eint64 mask, int block, size_t size);
...
int     _ETB_EXPORT etb_etcom_start_upload_ex(ETB *etb, int axis, int block, size_t *size);
...
eint64	_ETB_EXPORT etb_usb_reserved_function1(ETB *etb);
void	_ETB_EXPORT etb_usb_reserved_function2(ETB *etb);
void    _ETB_EXPORT etb_m63_to_m60m61(dword m63, dword *m60, dword *m61);
void    _ETB_EXPORT etb_m60m61_to_m63(dword m60, dword m61, dword *m63);
int     _ETB_EXPORT etb_ultimet_reserved_function1(ETB *etb, dword addr, volatile dword *value);
 ...
 *-----------------------------------------------------------------------------*/
    ...
    enum {EBADMESSAGE = -219 };                     /* a bad message is given */
 ...
 *-----------------------------------------------------------------------------*/
	...
	public:
		...
		void etcomStartDownloadEx(eint64 mask, int block, size_t size) {
			ERRCHK(etb_etcom_start_download_ex(etb, mask, block, size));
		}
		...
		void etcomStartUploadEx(int axis, int block, size_t *size) {
			ERRCHK(etb_etcom_start_upload_ex(etb, axis, block, size));
		}
		...
		void setPolicy(int policy, int prio) {
			ERRCHK(etb_set_policy(etb, policy, prio));
		}
		...
		void readSlotArray(int nbSlots, int slotArray[], dword *valuesArray[]) {
			ERRCHK(etb_read_slot_array(etb, nbSlots, slotArray, valuesArray));
		}
		...
		void writeSlotArray(int nbSlots, int slotArray[], dword valuesArray[]) {
			ERRCHK(etb_write_slot_array(etb, nbSlots, slotArray, valuesArray));
		}
 ...
 *-----------------------------------------------------------------------------*/
	...
	public:
		 ...
		 */
	public:
		void etcomPutMsg(eint64 mask, const EtbEtcom &rec, const void *key = NULL, long timeout = 0) {
			ERRCHK(etb_etcom_putm(port, key, mask, (const ETB_ETCOM *)&rec, timeout));
		}
		void etcomPutMsgUnsafe(eint64 mask, const EtbEtcom &rec, const void *key = NULL, long timeout = 0) {
			ERRCHK(etb_etcom_putm_unsafe(port, key, mask, (const ETB_ETCOM *)&rec, timeout));
		}
