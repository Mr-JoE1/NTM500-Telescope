...
#define TRA_EBADMESSAGE                  -552        /**< bad response from etel bus */
...
/* 64 bits integer			*/
	...
	#elif defined POSIX || defined VXWORKS_6_9
		#define eint64 long long
	#endif
#endif
...
int		_TRA_EXPORT tra_receive_direct_cmd_e_ex(TRA *tra, char_p buffer, int max, bool *p_ack, bool *p_error, bool *p_req, void **p_key, int timeout);
 ...
 *-----------------------------------------------------------------------------*/
    ...
    enum {EBADMESSAGE = -552 };                     /* bad response from etel bus */
 ...
 *-----------------------------------------------------------------------------*/
	...
	class TraAbstractTraductor;
		...
		typedef int TRA_CALLBACK TraIsoConverter(TraAbstractTraductor tra, int *incr, double *iso, int conv, int axis, bool to_iso, void *user);
#endif /* TRA_OO_API */
 ...
 *-----------------------------------------------------------------------------*/
	...
	class TraDirectTraductor : public TraOLDirectTraductor {
		...
		public:
			...
			void *receiveCmd(char_p buffer, int max, bool *p_ack, bool *p_error, bool *p_req, int timeout) {
				void *key;
				ERRCHK(tra_receive_direct_cmd_e_ex(tra, buffer, max, p_ack, p_error, p_req, &key, timeout));
				return key;
			}
